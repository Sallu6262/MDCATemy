...
const SavedCopy = ({savedMcqs, totalSaved, setSavedMcqs}) => {
    ...
    const displaySubjectButton = (subjectName, subjectNumber, count) => {
        return <button onClick={async () => {
            ...
            await subjectFilter(false, url, setURL, subjectName, setPageNumber, setTotalPages, setSavedMcqs, totalSaved, setSubject, filterByNumber[subjectName], setLoading);
    ...
    return (
        <section id="section-saved" className="fade-in space-y-4">
            ...
            <div className="flex items-center gap-2">
                <div onKeyDown={async (e) => {if(e.key === 'Enter') {await searchFilter(false, url, setURL, setPageNumber, setTotalPages, setSavedMcqs, search, setLoading)}}} className="relative flex-1">
                    ...
                    onChange={async e => {
                        ...
                        if(!val) await searchFilter(false, url, setURL, setPageNumber, setTotalPages, setSavedMcqs, search, setLoading);
                        setSearch(val);
                    }} 
                    type="text" placeholder="Search a topic..."
                ...
                <button
                    type="button"
                    onClick={async () => {await searchFilter(false, url, setURL, setPageNumber, setTotalPages, setSavedMcqs, search, setLoading)}}
                ...
                <div className="space-y-3">
                    {
                        ...
                        <>
                        {
                            savedMcqs?.map((savedMcq, i) => {
                                return <SavedMCQCard key={savedMcq.mcq_id} mcq={savedMcq} mcqNo={i+1}/>
                            }) 
                        }
                        <div className="flex items-center justify-between gap-3">
                            {
                                ...
                                <button
                                    type="button"
                                    onClick={async () => {await pageFilter(false, url, setURL, false, pageNumber, setPageNumber, setSavedMcqs, setLoading)}}
...

                            {
                                ...
                                <button
                                    type="button"
                                    onClick={async () => await pageFilter(false, url, setURL, true, pageNumber, setPageNumber, setSavedMcqs, setLoading)}

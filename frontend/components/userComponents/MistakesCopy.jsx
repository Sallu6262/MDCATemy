...
import ReTestPopUp from './ReTestPopUp';
import MCQListSkeleton from '../skeletonComponents/MCQListSkeleton';
...
const MistakesCopy = ({wrongMcqs, totalMistakes, pendingMistakes, setPendingMistakes, setWrongMcqs, totalWrong, setTotalWrong}) => {
    ...
    const [retestPopUpHidden, setRetestPopUpHidden] = useState(true);
    ...
    const displaySubjectButton = (subjectName, subjectNumber, count) => {
        return <button onClick={async () => {
                await subjectFilter(true, url, setURL, subjectName, setPageNumber, setTotalPages, setWrongMcqs, totalWrong, setSubject, filterByNumber[subjectName], setLoading);
        ...
        <>
            {
                !retestPopUpHidden && 
                <ReTestPopUp 
                    setRetestPopUpHidden={setRetestPopUpHidden} 
            ...
            <section id="section-mistakes" className="fade-in space-y-5">
...

                {
                    ...
                    <div className="flex justify-center">
                        <button onClick={() => setRetestPopUpHidden(false)} className="cursor-pointer flex items-center justify-center gap-2.5 px-8 py-4 rounded-2xl bg-[#FFC600] text-[#181A18] text-[15px] font-[Poppins] font-black uppercase tracking-wide hover:bg-[#FFC600]-dark transition-colors shadow-[0_0_28px_rgba(255,198,0,0.35)] w-full sm:w-auto sm:min-w-[320px]">
                ...
                <div className="space-y-3">
                    ...
                    <div onKeyDown={async (e) => {if(e.key === 'Enter') {await searchFilter(true, url, setURL, setPageNumber, setTotalPages, setWrongMcqs, search, setLoading)}}} className="flex-1 relative">
                        ...
                        onChange={async e => {
                            ...
                            if(!val) await searchFilter(true, url, setURL, setPageNumber, setTotalPages, setWrongMcqs, search, setLoading);
                            setSearch(val);
                        }}
                    ...
                    <button
                        type="button"
                        onClick={async () => {await searchFilter(true, url, setURL, setPageNumber, setTotalPages, setWrongMcqs, search, setLoading)}} 
...

                {
                    ...
                    <div className="space-y-3">

                        {
                            ...
                            <>
                            {
                                notMasteredMcqs?.map((mcq, i) => <WrongMCQCard key={mcq.mcq_id} mcq={mcq} setWrongMcqs={setWrongMcqs} setNotMasteredMcqs={setNotMasteredMcqs} setPendingMistakes={setPendingMistakes} totalWrong={totalWrong} setTotalWrong={setTotalWrong}/>)
                            }
                                <div className="flex items-center justify-between gap-3">
                                    ...
                                    <button
                                        type="button"
                                        onClick={async() => {await pageFilter(true, url, setURL, false, pageNumber, setPageNumber, setWrongMcqs, setLoading)}}
...

                                {
                                    ...
                                    <button
                                        type="button"
                                        onClick={async() => {await pageFilter(true, url, setURL, true, pageNumber, setPageNumber, setWrongMcqs, setLoading)}}

...
const TestSyllabusPopUp = ({test, setSyllabusHidden}) => {
    ...
    return (
        <>
            <section ref={testRef} tabIndex={-1} className="fade-in custom-mix-overlay absolute inset-0 z-[60] flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
                <div className="w-[calc(100%-24px)] max-w-[980px] max-h-[88vh] bg-[#222422] border border-[#2E302E] rounded-2xl overflow-hidden flex flex-col shadow-2xl">
                    ...
                    <button onClick={() => setSyllabusHidden(true)} className="cursor-pointer w-8 h-8 rounded-lg flex items-center justify-center text-[#A8ACA8] hover:text-white hover:bg-[#2A2C2A]/40 transition-colors flex-shrink-0">
...

                    {
                        Object.keys(test?.syllabus).map((subject,i) => {
                            return <div key={i} 
                                className={`bg-[#0E0F0E]/40 border border-[#2E302E] rounded-xl overflow-hidden border-l-[3px] ${subjectToColor('border-l', subject)}`}
                            >
                                <div className="w-full flex items-center justify-between px-3 py-2.5 text-left">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <p className="font-[Poppins] font-black text-white text-[18px]">{formatName(subject)}</p>
                                    </div>
                                </div>
                                    ...
                                    Object.keys(test?.syllabus[subject]).map((chapter, i) => {
                                        return (
                                            <div key={i} className="border-l border-[#2E302E] pl-3 ml-2">
                                                <p className="font-[Inter] font-bold text-white text-[14px] mb-1">{formatName(chapter)}</p>
                                                <div className="flex flex-wrap gap-1">
                                                {
                                                    test?.syllabus[subject][chapter]?.map((topic, i) => {
                                                        return (
                                                            <span key={i} className="text-[12px] font-[Inter] font-bold px-2.5 py-1 rounded-full bg-white/[0.04] border border-[#2E302E] text-[#A8ACA8]">{topic.name || topic}</span>                                              
                                                        )
                                                    })
                                                }
                                                </div>
                    ...
                    <button onClick={() => setSyllabusHidden(true)} className="cursor-pointer w-full py-2.5 bg-white/[0.06] border border-[#2E302E] rounded-xl text-white text-[14px] font-[Inter] font-bold hover:border-[#FFC600]/40 transition-colors">
                        Close
                    </button>
                    </div>

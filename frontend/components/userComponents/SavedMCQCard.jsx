...
const SavedMCQCard = ({mcq, mcqNo}) => {
    ...
    const showMCQOption = (correctOption, mcqOption, optionValue) => {
        return <div className={`flex items-start gap-3 px-3 py-2.5 rounded-lg border ${correctOption === mcqOption ? 'bg-emerald-500/10 border-emerald-500/40' : 'quiz-answer-inactive border-[#2E302E]/40'}`}>
            <span className={`flex-shrink-0 w-5 h-5 rounded-full flex items-center justify-center text-[13px] font-mono font-bold mt-[1px] ${mcqOption === correctOption ? 'bg-emerald-500 text-white' : 'copy-option-badge-inactive'}`}>{mcqOption}</span>
    ...
    return (
        <div ref={savedRef} tabIndex={-1} className="copy-mcq-card bg-[#222422] border border-[#2E302E] rounded-xl overflow-hidden">
            <div className="px-4 pt-4 pb-3">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <span className="copy-chapter-pill text-[13px] font-[Inter] text-[#A8ACA8] bg-[#2A2C2A]/20 px-2.5 py-1 rounded-full">{mcq.chapter_name}</span>
                </div>
                <div className="flex items-center gap-2 flex-shrink-0">
                    ...
                    <button onClick={deleteSavedMCQ} className={`quiz-btn-secondary ${deleteLoading ? 'cursor-not-allowed' : 'cursor-pointer'} w-7 h-7 flex items-center justify-center rounded-lg border text-[#A8ACA8] hover:text-red-400 transition-all`}>
            ...
            <button onClick={() => setAnswerHidden(prev => !prev)} className="copy-answer-toggle cursor-pointer w-full flex items-center justify-between px-4 py-2.5 border-t border-[#2E302E]/60 text-[#FFC600] bg-[#FFC600]/5 transition-all">
            ...
            <div className={`dropdown-menu ${!answerHidden ? 'open' : ''} px-4 pb-4 pt-1 space-y-3`}>
                ...
                <div className="copy-callout callout-yellow rounded-r-lg py-3 px-4 bg-[#181A18]">
                <p className="text-[13px] font-[Inter] font-bold text-[#FFC600] uppercase tracking-wide mb-1.5">Explanation</p>
                <p className="font-[Inter] text-[13px] text-white/85 leading-relaxed">

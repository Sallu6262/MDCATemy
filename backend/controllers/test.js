...
import { convertSyllabusQueryResultIntoSyllabusObject, formatColumnName, formatDate, readDataFromExcelFile } from "../helpers.js";
...
export const getAllPreviousTests = handleAsyncError(async (req, res, next) => {
    ...
    let result = (await pool.query("SELECT tests.test_id, tests.test_name, tests.test_date::TEXT AS test_date, tests.mcq_count, tests.test_time, SUM(CASE WHEN attempted_mcqs.selected_option = mcq_bank.correct_option THEN 1 ELSE 0 END)::INT AS correct, SUM(CASE WHEN attempted_mcqs.selected_option != mcq_bank.correct_option THEN 1 ELSE 0 END)::INT AS mistakes, SUM(CASE WHEN attempted_mcqs.student_id IS NULL THEN 1 ELSE 0 END)::INT AS skipped FROM tests LEFT JOIN test_mcqs ON test_mcqs.test_id=tests.test_id LEFT JOIN mcq_bank ON mcq_bank.mcq_id = test_mcqs.mcq_id LEFT JOIN attempted_mcqs ON attempted_mcqs.mcq_id = mcq_bank.mcq_id AND attempted_mcqs.test_id = test_mcqs.test_id AND attempted_mcqs.student_id=$1 WHERE tests.test_date <= CURRENT_DATE AND tests.test_id IN (SELECT test_id FROM test_enrollments WHERE student_id=$1) GROUP BY tests.test_id, tests.test_name, tests.test_date, tests.mcq_count, tests.test_time ORDER BY test_date DESC", [req.user.student_id])).rows;
    result = result.filter((obj) => {
        return obj.correct !== 0 || obj.mistakes !== 0 || obj.test_date !== formatDate(new Date());
    });
    res.status(200).json({
        status: "success",
        data: result

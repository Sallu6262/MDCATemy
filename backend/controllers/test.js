...
import { convertSubjectsChapterTopicsIntoNestedObject, formatColumnName, getSlug, readDataFromExcelFile } from "../helpers.js";
...
export const getAllUpcomingTests = handleAsyncError(async (req, res, next) => {
    ...
    let data = (await pool.query("SELECT test_id, test_name, slug, test_date, mcq_count, test_time, STRING_AGG(subject_name, ',' ORDER BY subject_name) AS subjects, STRING_AGG(chapters_per_subject, '&&&' ORDER BY subject_name) AS chapters, STRING_AGG(topics_per_subject, '^^^' ORDER BY subject_name) AS topics FROM (SELECT test_id, test_name, slug, test_date, mcq_count, test_time, subject_name, STRING_AGG(chapter_name, ',' ORDER BY chapter_name) AS chapters_per_subject, STRING_AGG(topics_per_chapter, '&&&' ORDER BY chapter_name) AS topics_per_subject FROM (SELECT tests.test_id AS test_id, test_name, slug, test_date::TEXT, mcq_count, test_time, subject_name, chapter_name, STRING_AGG(topic_name, ',' ORDER BY topic_name) AS topics_per_chapter FROM tests INNER JOIN test_topics ON tests.test_id=test_topics.test_id INNER JOIN topics ON test_topics.topic_id=topics.topic_id INNER JOIN subjects ON topics.subject_id=subjects.subject_id INNER JOIN chapters ON chapters.chapter_id=topics.chapter_id WHERE tests.test_date >= CURRENT_DATE AND tests.test_id IN (SELECT test_id FROM test_enrollments WHERE student_id=$1) GROUP BY tests.test_id, test_name, slug, test_date, mcq_count, test_time, subject_name, chapter_name ORDER BY tests.test_date) GROUP BY test_id, test_name, slug, test_date, mcq_count, test_time, subject_name) GROUP BY test_id, test_name, slug, test_date, mcq_count, test_time", [req.user.student_id])).rows;
